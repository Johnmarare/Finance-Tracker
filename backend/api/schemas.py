...
from api.models import User, Expense, Income, Budget
...
class ExpenseSchema(ma.SQLAlchemySchema):
    ...
    id = ma.auto_field(dump_only=True)
    url = ma.String(dump_only=True)
    description = ma.String(required=True, validate=validate.Length(
        min=1, max=50))
    amount = ma.Float(required=True, validate=validate.Range(min=0))
...
class BudgetSchema(ma.SQLAlchemySchema):
    class Meta:
        model = Budget
        include_fk = True
        ordered = True
    ...
    id = ma.auto_field(dump_only=True)
    url = ma.String(dump_only=True)
    category = ma.String(required=True, validate=validate.Length(
        min=1, max=50))
    assigned = ma.Float(required=True, validate=validate.Range(min=0))
    activity = ma.Float(required=True, validate=validate.Range(min=0))
    available = ma.Float(dump_only=True)
    timestamp = ma.auto_field(dump_only=True)
    author = ma.Nested(UserSchema, dump_only=True)

    @post_dump
    def calculate_available(self, data, **kwargs):
        # Calculate 'available' based on 'assigned' and 'activity'
        data['available'] = data['assigned'] - data['activity']
        return data
